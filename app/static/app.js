(() => {
  ...
  async function applySeamAllowance() { try { const allowance = Number($('#allowance-mm').value); const result = await api('/api/design/seam-allowance', { method: 'POST', body: JSON.stringify({ pattern: editor.project, allowanceMm: allowance }) }); loadEditorProject({ ...result.pattern, projectName: `${editor.project.projectName} / seam +${allowance}mm` }, false); setResult('#design-ops-result', `PASS\nseam allowance +${allowance.toFixed(1)} mm\n${result.patternHash}\n\nInspection-only derived pattern.`, 'pass'); toast('Seam allowance applied as a new project state.'); } catch (error) { setResult('#design-ops-result', error.message, 'fail'); } }
  async function applyGrade() { try { const piece = currentPiece(); const increments = { [piece.pieceId]: { xMm: Number($('#grade-x-mm').value), yMm: Number($('#grade-y-mm').value) } }; const result = await api('/api/design/grade', { method: 'POST', body: JSON.stringify({ pattern: editor.project, increments }) }); loadEditorProject({ ...result.pattern, projectName: `${editor.project.projectName} / grade` }, false); setResult('#design-ops-result', `PASS\ngraded ${piece.pieceId} by ${increments[piece.pieceId].xMm} × ${increments[piece.pieceId].yMm} mm\n${result.patternHash}\n\nExplicit translation only; no size rule inferred.`, 'pass'); toast('Explicit grade applied as a new project state.'); } catch (error) { setResult('#design-ops-result', error.message, 'fail'); } }
  async function compareEditor() { if (!editor.reference) { setResult('#design-ops-result', 'Load a reference JSON first.', 'fail'); return; } try { const result = await api('/api/design/compare', { method: 'POST', body: JSON.stringify({ reference: editor.reference, candidate: editor.project, toleranceMm: 0.1 }) }); setResult('#design-ops-result', `${result.status}\nmax point delta: ${result.report.maxPointDeltaMm} mm\ncompared points: ${result.report.comparedPoints}\n${result.report.structuralErrors.join('\n')}`, result.status === 'PASS' ? 'pass' : 'fail'); } catch (error) { setResult('#design-ops-result', error.message, 'fail'); } }
  async function checkConstraint() { try { const contour = currentContour(); const result = await api('/api/design/constraints', { method: 'POST', body: JSON.stringify({ pattern: editor.project, constraints: [{ pieceId: currentPiece().pieceId, contour: contour.name, fromIndex: 0, toIndex: 1, kind: 'distance', valueMm: Number($('#constraint-expected-mm').value), toleranceMm: Number($('#constraint-tolerance-mm').value) }] }) }); const row = result.report.constraints[0]; setResult('#design-ops-result', `${result.status}\n${row.actualMm} mm actual / ${row.expectedMm} mm expected\nΔ ${row.deltaMm} mm`, result.status === 'PASS' ? 'pass' : 'fail'); } catch (error) { setResult('#design-ops-result', error.message, 'fail'); } }
  async function downloadDxf() { try { const result = await api('/api/export/dxf', { method: 'POST', body: JSON.stringify({ pattern: editor.project }) }); const blob = new Blob([result.dxf], { type: 'application/dxf' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${editor.project.projectName.toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'pattern'}-inspection.dxf`; link.click(); URL.revokeObjectURL(link.href); setResult('#design-ops-result', `PASS\n${result.patternHash}\n\nDXF-style inspection export downloaded.\nNOT MANUFACTURING APPROVAL.`, 'pass'); } catch (error) { setResult('#design-ops-result', error.message, 'fail'); } }
  ...
  $('#design-reference-file').addEventListener('change', (event) => parseFile(event.target, (data) => { editor.reference = normaliseProject(data); $('#reference-file-status').textContent = `${data.projectName || 'Reference'} loaded locally.`; }));
